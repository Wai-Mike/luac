...
export default function YouthCensusEdit({ member }) {

    const { data, setData, put, processing, errors, transform } = useForm(toFormState(member));
    ...
    const submit = (e) => {
        e.preventDefault();
        if (!member) return;
        put(`/admin/youth-members/${member.id}`, { preserveScroll: true });
    };

    if (!member) {
        return (
            <AppLayout breadcrumbs={youthBreadcrumbs}>
                <Head title="Admin · Edit youth record" />
    ...
    const breadcrumbs = [
        ...youthBreadcrumbs,
        { title: `Edit · ${member.first_name ?? 'Record'}` },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit · ${member.first_name}`} />

            <div className="mx-auto max-w-2xl space-y-6">
                <Link href={`/admin/youth-members/${member.id}`} className="text-sm font-medium text-[rgb(29,84,114)] hover:underline">
                    ← Back to record
                </Link>
                <h1 className="text-2xl font-semibold text-slate-900">Edit youth member</h1>
